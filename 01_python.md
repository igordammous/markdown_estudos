...
1. **Listas**: Uma lista pode conter dados como `nomes` de pessoas, `idades` ou até mesmo `notas`. Por exemplo, você pode ter uma lista de nomes e usar um `for` para imprimir cada `nome`.
...
2. **Dicionários**: Um dicionário é uma estrutura que armazena pares de chave-valor. Você pode iterar sobre as chaves ou os valores. Por exemplo, se você tiver um dicionário com notas de alunos, pode usar um for para acessar cada aluno e sua respectiva nota.
...
3.	**Conjuntos**: Um conjunto é uma coleção de elementos únicos. Você pode usar um for para iterar sobre os elementos de um conjunto e, por exemplo, contar quantos elementos únicos você tem.
